    ...
    def get(self, request):
        ...
        try:
            if uid:
                ...
                Address_serializer = Address_listModelSerializer(a_list,
                                                                 many=True)
                mes['code'] = 200
...
class Type_move(APIView):
    ...
    def get(self, request):
        ...
        assemble_tserializer = Assemble_typeModelSerializer(assemble_t_list,
                                                            many=True)
        mes['code'] = 200
...
class Add_move(APIView):
    ...
    def post(self, request):
        ...
        if len(datas) >= 11:
            try:
                Move_into.objects.create(user_id=uid,
                                         name=name,
                                         idcard=idcard,
                                         age=age,
                                         tep=tep,
                ...
                Company.objects.create(user_id=uid,
                                       name=cname,
                                       pic=cpic,
                                       details=ccontent,
                                       t_id=t_id).save()
...
class Add_moves(APIView):
    ...
    def post(self, request):
        ...
        if len(datas) >= 13:
            try:
                Move_intos.objects.create(user_id=uid,
                                          name=name,
                                          idcard=idcard,
                                          names=names,
                                          age=age,
                ...
                num = ninthnum()
                Users.objects.create(tep=tep, password=pwd, mmh=num,
                                     name=tep).save()
...
class Recommend(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            gcompany_list_tserializer = Gcompany_listModelSerializer(
                gcompany_list, many=True)
            mes['code'] = 200
...
class Announcements(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            Announcement_list_tserializer = Announcement_listModelSerializer(
                announcement_list, many=True)
            mes['code'] = 200
...
class Clauseinfo(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            clause_list_tserializer = Clause_listModelSerializer(clause_list,
                                                                 many=True)
            mes['code'] = 200
...
class Searchs(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            st_data = Search_take_notes.objects.filter(
                name=name).order_by('id').first()
            num += int(st_data.nums)
            Search_take_notes.objects.filter(name=name).update(nums=num)

            goods_data = Goods.objects.filter(Q(name__icontains=name),
                                              Q(is_del=0))
...
class Search_takeinfo(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            search_take_notes_list_tserializer = Search_take_notesModelSerializer(
                search_take_notes_list, many=True)
            mes['code'] = 200
...
class Search_lenovo(APIView):
    ...
    def get(self, request):
        ...
        try:
            st_data = Search_take_notes.objects.filter(
                name__istartswith=name).order_by('-nums').values(
                    'name').distinct()[:5]
            mes['code'] = 200
            mes['data'] = st_data
        except:
...
class Users_address_type(APIView):
    ...
    def post(self, request):
        ...
        try:
            Users_address.objects.filter(type=1,
...
class Goods_list(APIView):
    ...
    def post(self, request):
        ...
        citylist=splits(citylist)
        goods_list=[]
        for i in citylist:
            q = dfilter(requset_data)
            q.add(Q(**{'city': i}), Q.AND)
            goods_val = Goods.objects.filter(q)
            goods_ser = GoodsModelSerializer(goods_val, many=True)
            goods_list.append(goods_ser.data)
        goods_datas=[]
        for j in goods_list:
            goods_datas.append(tags(j))
        mes['code'] = 200
        mes['data'] = goods_datas
        return Response(mes)
...
class Goods_details(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            is_collection = '未收藏'
            uc_data = User_collection.objects.filter(uid=uid,
                                                     tid=co_data.t_id,
                                                     g_id=gid)
            if uc_data:
                is_collection = '收藏'

            s_c_list = Goods_comment_types.objects.filter(
                g_id=gid).values_list('t_id', 'type_count')
            s_c_lists = []
            for i in s_c_list:
                s_c_dict = {}
            ...
            qa = QA.objects.filter(g_id=gid).values('id', 'questions',
                                                    'answers')
            qa_list = []
            if qa:
                for j in qa:
            ...
            s_t_list = Goods.objects.filter(
                Q(company_id=c_id[0]['company_id']),
                Q(is_del=0)).order_by('-sales_volume')[:3]
...
class Goods_norms_data(APIView):
    ...
    def get(self, request):
        ...
        gid = request.GET.get('g_id')
        try:
            goods_data = Goods_attribute_definition.objects.filter(
                g_id=gid).values_list('id', 'name')
            if not goods_data:
                return Response({'code': 0, 'msg': '暂无规格'})
            gdlist = []
            for i in goods_data:
                gdict = {}
                gas_data = Goods_attribute.objects.filter(
                    tid=i[0]).values('name')
                if not gas_data:
                    return Response({'code': 0, 'msg': '暂无数据'})
                gdict[i[1]] = gas_data
                gdlist.append(gdict)
            gn_data = Goods_norms.objects.filter(g_id=gid)
            if not gn_data:
                return Response({'code': 0, 'msg': '暂无规格详情'})
            gn_list = []
            for i in gn_data:
                gn_lis = []
                ids = splits(i.a_id)
                ...
                for j in range(len(ids)):
                    gn_dic = {}
                    ga_data = Goods_attribute.objects.filter(id=ids[j]).values(
                        'name', 'tid')
                    if not ga_data:
                        continue
                    else:
                        gad_data = Goods_attribute_definition.objects.filter(
                            id=ga_data[0]['tid'])
                        gad_ser = GadSerializer(gad_data, many=True)
                        gn_dic[gad_ser.data[0]['name']] = ga_data[0]['name']
                        gn_lis.append(gn_dic)
                gn_lis.append(i.price)
                gn_lis.append(i.stock)
                gn_list.append(gn_lis)
                # {颜色:{尺码:31,吃码:32}}
                # is[[gn_id,颜色,吃码,价格,库存],[gn_id,颜色,吃码,价格,库存],[gn_id,颜色,吃码,价格,库存]]
            datas = {'data': gdlist, 'norms': gn_list}
            mes['code'] = 200
            mes['data'] = datas
        except:
            mes['code'] = 0
...
class Goods_details_comment(APIView):
    ...
    def get(self, request):
        ...
        s_c_list = Goods_comment_types.objects.filter(g_id=gid).values_list(
            't_id', 'type_count')
        s_c_lists = []
        for i in s_c_list:
            s_c_dict = {}
        ...
        for i in c_dateinfo:
            c_ddict = {}
            u_data = Users.objects.filter(id=i.com_users_id).values(
                'name', 'pic')
        ...
        datas = {'rank': nums, 'comment': s_c_lists, 'data': c_dlist}
        mes['code'] = 200
        mes['data'] = datas
        return Response(mes)
...
class Goods_details_typeinfo(APIView):
    ...
    def get(self, request):
        ...
        try:
            c_dateinfo = Comment.objects.filter(g_id=gid,
                                                t_id=tid).order_by('-rank')
            c_dlist = []
            for i in c_dateinfo:
                c_ddict = {}
                u_data = Users.objects.filter(id=i.com_users_id).values(
...
class Submit_order_info(APIView):
    def get(self, request):
        ...
        try:
            ...
            gn_data=Goods_norms.objects.filter(id=gn_id).values('id','price','a_id')

            ua_val = Users_address.objects.filter(user_id=user_id, type=1)
            ua_ser = Users_addressSerializer(ua_val, many=True)

            goods_data = Goods.objects.filter(id=gid)
            c_data=Company.objects.filter(id=goods_data[0].company_id).values('name')

            goods_data[0].price=gn_data[0]['price']
            goods_ser=GoodsbfSerializer(goods_data, many=True)

            gn_lis = []
            ids = splits(gn_data[0]['a_id'])
            for j in range(len(ids)):
                gn_dic = {}
                ga_data = Goods_attribute.objects.filter(id=ids[j]).values(
                    'name', 'tid')
                if not ga_data:
                    ...
                    gn_lis.append(gn_dic)
            datas = {
                'company':c_data[0]['name'],
                'address': ua_ser.data,
                'goods_data':goods_ser.data,
                'norms':gn_lis,
                'nums':nums
            }
...
class Goods_add(APIView):
    ...
    def post(self, request):
        ...
        normsid=splits(normsid)
        norms_priceid = request_data.get('norms_priceid')
        norms_priceid=splits(norms_priceid)
        ...
        try:
            ...
            if int(request_data.get('type')) == 1:
                drill = Move_into.objects.filter(
                    user_id=user_id).values('drill')[0]['drill']
                crown = Move_into.objects.filter(
                    user_id=user_id).values('crown')[0]['crown']
                tid = Move_into.objects.filter(
                    user_id=user_id).values('t_id')[0]['t_id']
            elif int(request_data.get('type')) == 2:
                drill = Move_intos.objects.filter(
                    user_id=user_id).values('drill')[0]['drill']
                crown = Move_intos.objects.filter(
                    user_id=user_id).values('crown')[0]['crown']
                tid = Move_intos.objects.filter(
                    user_id=user_id).values('t_id')[0]['t_id']
            else:
                mes['code'] = 0
                mes['msg'] = 'type参数未添加'
            ...
            if gid:
                ...
                if goods_ser.is_valid():
                    ...
                    for i in normsid:
                        Goods_attribute_definition.objects.filter(id=i).update(
                            g_id=user_obj.id)
                    for i in norms_priceid:
                        Goods_norms.objects.filter(id=i).update(
                            g_id=user_obj.id)
                    for i in qaid:
                        QA.objects.filter(id=i).update(g_id=user_obj.id)
                    gn_data=Goods_norms.objects.filter(g_id=user_obj.id).order_by('price')[0]
                    user_obj.price=gn_data.price
                    user_obj.save()
                    mes['code'] = 200
                    mes['msg'] = '成功'
                else:
...
class Goods_attribute_info(APIView):
    ...
    def get(self, request):
        ...
        try:
            ...
            tidlist = []
            for i in tid:
                gad_da = Goods_attribute_definition.objects.filter(
                    id=i).values('id', 'name', 'g_id')
                if not gad_da:
                    return Response({'code': 200, 'mes': '暂无数据'})
                gad_ser = GadSerializer(gad_da, many=True)
                gad_data = gad_ser.data
                ga_da = Goods_attribute.objects.filter(tid=i).values(
                    'id', 'name', 'tid')
...
class Catering_info_add(APIView):
    ...
    def post(self, request):
        ...
        if int(request_data.get('type')) == 1:
            drill = Move_into.objects.filter(
                user_id=uid).values('drill')[0]['drill']
            crown = Move_into.objects.filter(
                user_id=uid).values('crown')[0]['crown']
        elif int(request_data.get('type')) == 2:
            drill = Move_intos.objects.filter(
                user_id=uid).values('drill')[0]['drill']
            crown = Move_intos.objects.filter(
...
class Catering_goods_add(APIView):
    ...
    def post(self, request):
        ...
        drill, crown = 0, 0
        try:
            drill = Catering_shop.objects.filter(
                id=cid).values('drill')[0]['drill']
            crown = Catering_shop.objects.filter(
                id=cid).values('crown')[0]['crown']
            pdict = {'drill': drill, 'crown': crown}
            for i in request_data:
                rdict = {}
                if type(request_data[i]) == str:
